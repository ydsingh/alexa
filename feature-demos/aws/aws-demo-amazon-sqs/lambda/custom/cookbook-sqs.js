...
function buildGetQueueAttributesPayload(queueUrl, attributes) {
  return {
    QueueUrl: queueUrl,
...
class SQSQueue {
  ...
  async getQueueAttributes(attributes) {
    try {
      const queueUrl = await this.getQueueUrl();
      const payload = buildGetQueueAttributesPayload(queueUrl, attributes);
